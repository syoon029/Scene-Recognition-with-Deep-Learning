...
class ImageLoader(data.Dataset):
    ...
    def load_imagepaths_with_labels(
        ...
        for class_n in class_labels:
            class_p = os.path.join(self.curr_folder, class_n)
            class_i = class_labels[class_n]
            
            for file_n in os.listdir(class_p):
                img_p = os.path.join(class_p, file_n)
                img_paths.append((img_p, class_i))
    ...
    def get_classes(self) -> Dict[str, int]:
        ...
        class_folders = sorted(os.listdir(self.curr_folder))
        
        for i, class_n in enumerate(class_folders):
            classes[class_n] = i
        ############################################################################
        # Student code end
        ############################################################################
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        ...
        # Student code start
        ############################################################################
        img_p, class_idx = self.dataset[index]
        img = self.load_img_from_path(img_p)
